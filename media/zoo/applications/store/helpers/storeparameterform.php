 ...
 * @since 2.0
 */
class StoreParameterFormHelper extends AppHelper {
	 ...
	 * @since 2.0
	 */
	public function create($args = array(), $group = null) {
		$args = (array) $args;
		array_unshift($args, $this->app);
		$args[] = $group;
		return $this->app->object->create('StoreParameterForm', $args);
	}
...
class StoreParameterForm {
	 ...
	 * @since 2.0
	 */
	public function __construct($app, $xml = null, $group = null) {

		// init vars
		$this->app = $app;
		$this->loadXML($xml, $group);
	}
	 ...
	 * @since 2.0
	 */
	public function loadXML($xml, $group = 'default') {
		...
		if ($element || ($element = @simplexml_load_file($xml)) || ($element = simplexml_load_string($xml))) {
			if (isset($element->form)) {
				foreach ($element->form as $form) {
					if($form->attributes()->group == $group) {
	...
	public function render($control_name = 'params', $group = '_default') {
		...
		foreach ($this->_xml[$group]->param as $param) {
			...
			$width = (string) $param->attributes()->width;
			$width = $width ? $width : '1-1';
			$value = $this->getValue((string) $param->attributes()->name, (string) $param->attributes()->default);
			$class = 'uk-width-1-1';

			$field = '<div class="field">'.$this->app->field->render($type, $name, $value, $param, array('control_name' => $control_name, 'parent' => $this, 'class' => $class)).'</div>';
