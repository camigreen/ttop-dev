	...
	var XmlPrecaching = function() {
		...
		var addListeners = function(initialize) {
			...
			if(initialize) {
				...
				$('#language_option, #menu_datasource_filters').on('change.precaching', function(jqEvent, isTriggered){
		...
		var runProcessing = function() {
			...
			var iterationPromise = $.Deferred(function(defer) {
				$.ajax({
					type : "GET",
					url : "../" + selectedLanguage + "?option=com_jmap&task=sitemap.doPreCaching",
					dataType : 'json',
					context : this,
					data : postedParams
		...
		var setPrecachedStatusLabels = function() {
			...
			if($('input[data-role=sitemap_links]').length) {
				var availableSitemapLinksWrappedSet = $('input[data-role=sitemap_links]').slice(1, 5);
				$(availableSitemapLinksWrappedSet).each(function(index, value){
					availableSitemapLinks[index] = $(value).val();
				});
			} else {
				//SEF links mode detected
				var availableSitemapLinksWrappedSet = $('input[data-role=sitemap_links_sef]').slice(1, 5);
				$(availableSitemapLinksWrappedSet).each(function(index, value){
