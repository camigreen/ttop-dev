...
(function($) {
	var Analyzer = function() {
		...
		var addListeners = function(initialize) {
			...
			$('#adminForm li[class!=active][class!=disabled] a').on('click.analyzer', function(jqEvent){
				// Show striped progress started generation
		...
		var startSitemapCaching = function(targetSitemapLink) {
			...
			dataSourcePromise.then(function() {
				...
				window.location.href = 'index.php?option=com_jmap&task=analyzer.display&jsclient=1' + sitemapLang + sitemapMenuID;
