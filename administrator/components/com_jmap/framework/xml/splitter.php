...
/**
 * @package JMAP::LIBRARIES::components::com_jmap
...
interface IJMapXmlSplitter {
	 ...
	 * @return void
	 */
	public function chunkXMLString($xmlString, $tag = 'url', $howmany = 5);
}
...
class JMapXmlSplitter implements IJMapXmlSplitter {
	...
	private function processChunk($xmlstring) {
		...
		$name = "sitemap_" . $this->format . $this->language . '_' . $this->chunksCounter . '.xml';
	...
	private function generateIndexFile() {
		...
		$name = "sitemapindex_" . $this->format . $this->language . '.xml';
		$file = array('data' => $data, 'name' => $name);
