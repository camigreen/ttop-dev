...
class plgSystemNextendLibrary extends JPlugin {
	...
	$cachetime = $this->_data->get('cachetime', 'static');
	if ($cachetime != 0) {
	...
	if ($cachepath != '') {
		...
		$path = realpath(JPATH_PLATFORM . '/..');
	    } else {
		$path = JPATH_SITE;
	    }
	    $cachepath = rtrim($path, DIRECTORY_SEPARATOR) . str_replace('/', DIRECTORY_SEPARATOR, $cachepath);
