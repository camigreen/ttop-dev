...
class JMapModelSitemap extends JMapModel implements IJMapModelSitemap, IJMapModelExportable {
	...
	private function getSourceData($source, $accessLevels) {
		...
		if(!$resultSourceObject->params->get('xmlmobileinclude', 0) && $this->documentFormat == 'mobile') {
			return false;
		}
		...
		switch ($source->type) {
			...
			case 'content':
				...
				if($format == 'html') {
					$htmlSelectFields = 'c.title as title, cat.title as category, cat.level,';
				}
				...
				$contentQueryItems = "SELECT c.*, cat.id AS catid," .
									 $htmlSelectFields .
	...
	function __construct($config = array()) {
		...
		if(isset($config['iteration_counter'])) {
			$formatLimitParam = $this->documentFormat == 'images' ? $this->cparams->get('precaching_limit_images', 50) : $this->cparams->get('precaching_limit_xml', 5000);
